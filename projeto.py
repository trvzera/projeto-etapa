...
def consultarEstoque():
    ...
    if info:
        ...
        if escolha == "s":
            ...
            elif info['pais'] == "ARGENTINA":
                print(f"Preço em Peso: ${info['preco'] * taxaPeso:.2f}")
    else:
...
def listarEstoque():
    ...
    for codigo, info in estoque.items():
        ...
        pais = info.get('pais', '-')
        compat = info.get('compatibilidade', '-')
        print(f"[{codigo}] {info['produto']:20} | Qtd: {info['quantidade']:3} | Preço: ${info['preco']:7.2f} | Tam: {tam} | País: {pais} | Comp: {compat}")
    print("="*50)
